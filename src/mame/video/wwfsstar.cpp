...
void wwfsstar_state::draw_sprites(bitmap_argb32 &bitmap, const rectangle &cliprect )
{
	/*- SPR RAM Format -**
	...
	**- End of Comments -*/
	...
	while (source < finish)
	{
		...
		if (enable)
		{
			...
			int colourbank = (source [1] & 0x00f0) >> 4;
...
uint32_t wwfsstar_state::screen_update(screen_device &screen, bitmap_argb32 &bitmap, const rectangle &cliprect)
{
