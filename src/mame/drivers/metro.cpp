...
u8 metro_base_state::irq_cause_r(offs_t offset)
{
...
void metro_base_state::update_irq_state()
{
...
uint8_t metro_base_state::irq_vector_r(offs_t offset)
{
	// logerror("%s: irq callback returns %04X\n", machine().describe_context(), m_irq_vectors[offset]);
	return m_irq_vectors[offset] & 0xff;
}

void metro_base_state::cpu_space_map(address_map &map)
{
	map(0xfffff0, 0xffffff).r(FUNC(metro_base_state::irq_vector_r)).umask16(0x00ff);
}


void metro_base_state::irq_cause_w(offs_t offset, u8 data)
{
...
void metro_base_state::device_timer(emu_timer &timer, device_timer_id id, int param, void *ptr)
{
	switch (id)
	{
	...
	case TIMER_MOUJA_IRQ:
		...
		break;
	default:
		assert_always(false, "Unknown id in metro_base_state::device_timer");
	}
}

WRITE_LINE_MEMBER(metro_base_state::vblank_irq)
{
	if (state)
	{
...
void metro_vdp1_state::trigger_eof(int state)
{
	m_vdp->screen_eof(state);
}

void metro_vdp2_state::trigger_eof(int state)
{
	m_vdp->screen_eof(state);
}

void metro_vdp3_state::trigger_eof(int state)
{
	m_vdp->screen_eof(state);
}

INTERRUPT_GEN_MEMBER(metro_base_state::periodic_interrupt)
{
	m_requested_int[4] = 1;
	update_irq_state();
}

TIMER_DEVICE_CALLBACK_MEMBER(metro_vdp2_state::bangball_scanline)
{
	int scanline = param;
...
WRITE_LINE_MEMBER(metro_base_state::karatour_vblank_irq)
{
	if (state)
	{
		m_requested_int[m_vblank_bit] = 1;
...
WRITE16_MEMBER(metro_base_state::mouja_irq_timer_ctrl_w)
{
...
WRITE_LINE_MEMBER(metro_base_state::puzzlet_vblank_irq)
{
	if (state)
	{
		m_requested_int[m_vblank_bit] = 1;
...
READ_LINE_MEMBER(metro_base_state::rxd_r)
{
...
WRITE8_MEMBER(metro_base_state::soundlatch_w)
{
...
READ8_MEMBER(metro_base_state::soundstatus_r)
{
	return (m_busy_sndcpu ? 0x00 : 0x01);
}

CUSTOM_INPUT_MEMBER(metro_base_state::custom_soundstatus_r)
{
	return (m_busy_sndcpu ? 0x01 : 0x00);
}

WRITE8_MEMBER(metro_base_state::soundstatus_w)
{
	m_soundstatus = data & 0x01;
}

template<int Mask>
WRITE8_MEMBER(metro_base_state::upd7810_rombank_w)
{
	m_audiobank->set_entry((data >> 4) & Mask);
}


READ8_MEMBER(metro_base_state::upd7810_porta_r)
{
	return m_porta;
}

WRITE8_MEMBER(metro_base_state::upd7810_porta_w)
{
	m_porta = data;
}

WRITE8_MEMBER(metro_base_state::upd7810_portb_w)
{
	/* port B layout:
...
WRITE8_MEMBER(metro_base_state::daitorid_portb_w)
{
	/* port B layout:
	   7 !clock latch for message to main CPU
	   6 !clock YM2151 I/O
...
WRITE8_MEMBER(metro_base_state::coin_lockout_1word_w)
{
...
WRITE16_MEMBER(metro_base_state::coin_lockout_4words_w)
{
...
WRITE_LINE_MEMBER(metro_base_state::vdp_blit_end_w)
{
...
void metro_base_state::upd7810_map(address_map &map)
{
...
void metro_base_state::ymf278_map(address_map &map)
{
...
READ16_MEMBER(metro_vdp2_state::balcube_dsw_r)
{
...
void metro_vdp2_state::balcube_map(address_map &map)
{
	...
	map(0x400000, 0x41ffff).r(FUNC(metro_vdp2_state::balcube_dsw_r));                             // DSW x 3
	map(0x500000, 0x500001).portr("IN0");                                // Inputs
	map(0x500002, 0x500003).portr("IN1");                                //
	map(0x500006, 0x500007).nopr();                                         //
	map(0x500002, 0x500009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
	map(0x600000, 0x67ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x6788a3, 0x6788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
...
void metro_vdp2_state::daitoa_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x100000, 0x17ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x1788a3, 0x1788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	...
	map(0x200002, 0x200009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
	map(0x300000, 0x31ffff).r(FUNC(metro_vdp2_state::balcube_dsw_r));                             // DSW x 3
...
void metro_vdp2_state::bangball_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	...
	map(0xc00000, 0xc1ffff).r(FUNC(metro_vdp2_state::balcube_dsw_r));                             // DSW x 3
	map(0xd00000, 0xd00001).portr("IN0");                                // Inputs
	map(0xd00002, 0xd00003).portr("IN1");                                //
	map(0xd00006, 0xd00007).nopr();                                         //
	map(0xd00002, 0xd00009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
	map(0xe00000, 0xe7ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0xe788a3, 0xe788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
...
void metro_vdp2_state::batlbubl_map(address_map &map)
{
	...
	map(0x200002, 0x200009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
	map(0x300000, 0x31ffff).r(FUNC(metro_vdp2_state::balcube_dsw_r));                             // read but ignored?
	map(0x400001, 0x400001).r("ymf", FUNC(ymf278b_device::read));   // Sound
...
void metro_vdp2_state::msgogo_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x100000, 0x17ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x1788a3, 0x1788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w));  // IRQ Cause / IRQ Acknowledge
	...
	map(0x200006, 0x200007).nopr();                                         //
	map(0x200002, 0x200009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
	map(0x300000, 0x31ffff).r(FUNC(metro_vdp2_state::balcube_dsw_r));                             // 3 x DSW
	map(0x400001, 0x400001).r("ymf", FUNC(ymf278b_device::read));   // Sound
	map(0x400000, 0x40000b).w("ymf", FUNC(ymf278b_device::write)).umask16(0x00ff); //
	map(0xf00000, 0xf0ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
}
...
void metro_vdp2_state::daitorid_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400000, 0x47ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x4788a3, 0x4788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	map(0x4788a4, 0x4788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0x4788a9, 0x4788a9).w(FUNC(metro_vdp2_state::soundlatch_w));                       // To Sound CPU
	map(0x800000, 0x80ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
	map(0xc00000, 0xc00001).portr("IN0");
	map(0xc00001, 0xc00001).w(FUNC(metro_vdp2_state::soundstatus_w));  // To Sound CPU
	map(0xc00002, 0xc00003).portr("IN1");                                //
	map(0xc00004, 0xc00005).portr("DSW0");                               //
	map(0xc00006, 0xc00007).portr("IN2");                                //
	map(0xc00002, 0xc00009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
}
...
void metro_vdp2_state::dharma_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400000, 0x40ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
	map(0x800000, 0x87ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x8788a3, 0x8788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	map(0x8788a4, 0x8788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0x8788a9, 0x8788a9).w(FUNC(metro_vdp2_state::soundlatch_w));                       // To Sound CPU
	map(0xc00000, 0xc00001).portr("IN0");
	map(0xc00001, 0xc00001).w(FUNC(metro_vdp2_state::soundstatus_w));  // To Sound CPU
	map(0xc00002, 0xc00003).portr("IN1");                                //
	map(0xc00004, 0xc00005).portr("DSW0");                               //
...
void metro_vdp1_state::karatour_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x400001, 0x400001).rw(FUNC(metro_vdp1_state::soundstatus_r), FUNC(metro_vdp1_state::soundstatus_w)); // From Sound CPU
	map(0x400002, 0x400003).portr("IN0");                                // Inputs
	map(0x400003, 0x400003).w(FUNC(metro_vdp1_state::coin_lockout_1word_w));               // Coin Lockout
	...
	map(0x8788a3, 0x8788a3).rw(FUNC(metro_vdp1_state::irq_cause_r), FUNC(metro_vdp1_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	map(0x8788a4, 0x8788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0x8788a9, 0x8788a9).w(FUNC(metro_vdp1_state::soundlatch_w));                       // To Sound CPU
	map(0xf00000, 0xf0ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
}
...
void metro_vdp2_state::kokushi_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
...
void metro_vdp1_state::lastfort_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400000, 0x40ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
	map(0x800000, 0x87ffff).m(m_vdp, FUNC(imagetek_i4100_device::map));
	...
	map(0x8788a4, 0x8788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0x8788a9, 0x8788a9).w(FUNC(metro_vdp1_state::soundlatch_w));                       // To Sound CPU
	map(0xc00001, 0xc00001).rw(FUNC(metro_vdp1_state::soundstatus_r), FUNC(metro_vdp1_state::soundstatus_w)); // From / To Sound CPU
	map(0xc00003, 0xc00003).w(FUNC(metro_vdp1_state::coin_lockout_1word_w));               // Coin Lockout
...
void metro_vdp1_state::lastforg_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400001, 0x400001).rw(FUNC(metro_vdp1_state::soundstatus_r), FUNC(metro_vdp1_state::soundstatus_w)); // From / To Sound CPU
	map(0x400002, 0x400003).portr("IN0");                                // Inputs
	map(0x400003, 0x400003).w(FUNC(metro_vdp1_state::coin_lockout_1word_w));               // Coin Lockout
	map(0x400004, 0x400005).portr("IN1");                                //
	map(0x400006, 0x400007).portr("DSW0");                               //
	...
	map(0x8f88a3, 0x8f88a3).rw(FUNC(metro_vdp1_state::irq_cause_r), FUNC(metro_vdp1_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	map(0x8f88a4, 0x8f88a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0x8f88a9, 0x8f88a9).w(FUNC(metro_vdp1_state::soundlatch_w));                       // To Sound CPU
...
void metro_base_state::gakusai_oki_bank_set()
{
	int bank = (m_gakusai_oki_bank_lo & 7) + (m_gakusai_oki_bank_hi & 1) * 8;
	m_oki->set_rom_bank(bank);
}

WRITE8_MEMBER(metro_base_state::gakusai_oki_bank_hi_w)
{
	m_gakusai_oki_bank_hi = data;
	gakusai_oki_bank_set();
}

WRITE8_MEMBER(metro_base_state::gakusai_oki_bank_lo_w)
{
...
READ16_MEMBER(metro_base_state::gakusai_input_r)
{
...
READ8_MEMBER(metro_base_state::gakusai_eeprom_r)
{
	return m_eeprom->do_read() & 1;
}

WRITE8_MEMBER(metro_base_state::gakusai_eeprom_w)
{
	// latch the bit
...
void metro_vdp3_state::gakusai_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x200000, 0x27ffff).m(m_vdp, FUNC(imagetek_i4300_device::v3_map));
	...
	map(0x278833, 0x278833).rw(FUNC(metro_vdp3_state::irq_cause_r), FUNC(metro_vdp3_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	map(0x278836, 0x278837).nopr().w("watchdog", FUNC(watchdog_timer_device::reset16_w));
	map(0x278880, 0x278881).r(FUNC(metro_vdp3_state::gakusai_input_r));                           // Inputs
	map(0x278882, 0x278883).portr("IN0");                                //
	map(0x278888, 0x278889).writeonly().share("input_sel");                 // Inputs
	map(0x400000, 0x400001).nopw();                                        // ? 5
	map(0x500001, 0x500001).w(FUNC(metro_vdp3_state::gakusai_oki_bank_lo_w));           // Sound
	...
	map(0xc00001, 0xc00001).rw(FUNC(metro_vdp3_state::gakusai_eeprom_r), FUNC(metro_vdp3_state::gakusai_eeprom_w));      // EEPROM
	map(0xd00001, 0xd00001).w(FUNC(metro_vdp3_state::gakusai_oki_bank_hi_w));
	map(0xf00000, 0xf0ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
}
...
void metro_vdp3_state::gakusai2_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x600000, 0x67ffff).m(m_vdp, FUNC(imagetek_i4300_device::v3_map));
	...
	map(0x678833, 0x678833).rw(FUNC(metro_vdp3_state::irq_cause_r), FUNC(metro_vdp3_state::irq_cause_w));  // IRQ Cause / IRQ Acknowledge
	map(0x678836, 0x678837).nopr().w("watchdog", FUNC(watchdog_timer_device::reset16_w));
	map(0x678880, 0x678881).r(FUNC(metro_vdp3_state::gakusai_input_r));                           // Inputs
	map(0x678882, 0x678883).portr("IN0");                                //
	map(0x678888, 0x678889).writeonly().share("input_sel");                 // Inputs
	map(0x800000, 0x800001).nopw();                                        // ? 5
	map(0x900001, 0x900001).w(FUNC(metro_vdp3_state::gakusai_oki_bank_lo_w));           // Sound bank
	map(0xa00001, 0xa00001).w(FUNC(metro_vdp3_state::gakusai_oki_bank_hi_w));           //
	...
	map(0xe00001, 0xe00001).rw(FUNC(metro_vdp3_state::gakusai_eeprom_r), FUNC(metro_vdp3_state::gakusai_eeprom_w));      // EEPROM
	map(0xf00000, 0xf0ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
}
...
READ8_MEMBER(metro_base_state::dokyusp_eeprom_r)
{
	// clock line asserted: write latch or select next bit to read
...
WRITE8_MEMBER(metro_base_state::dokyusp_eeprom_bit_w)
{
	// latch the bit
	m_eeprom->di_write(BIT(data, 0));
...
WRITE8_MEMBER(metro_base_state::dokyusp_eeprom_reset_w)
{
	// reset line asserted: reset.
	m_eeprom->cs_write(BIT(data, 0) ? ASSERT_LINE : CLEAR_LINE);
}

void metro_vdp3_state::dokyusp_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x200000, 0x27ffff).m(m_vdp, FUNC(imagetek_i4300_device::v3_map));
	map(0x278810, 0x27881f).writeonly().share("irq_levels");                // IRQ Levels
	...
	map(0x600000, 0x600003).w("ymsnd", FUNC(ym2413_device::write)).umask16(0x00ff);
	map(0x700001, 0x700001).rw(m_oki, FUNC(okim6295_device::read), FUNC(okim6295_device::write));  // Sound
	map(0xc00001, 0xc00001).w(FUNC(metro_vdp3_state::dokyusp_eeprom_reset_w));                      // EEPROM
	map(0xd00001, 0xd00001).rw(FUNC(metro_vdp3_state::dokyusp_eeprom_r), FUNC(metro_vdp3_state::dokyusp_eeprom_bit_w));  // EEPROM
	map(0xf00000, 0xf0ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
}
...
void metro_vdp3_state::dokyusei_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400000, 0x47ffff).m(m_vdp, FUNC(imagetek_i4300_device::v3_map));
...
//  map(0x478833, 0x478833).rw(FUNC(metro_vdp3_state::irq_cause_r), FUNC(metro_vdp3_state::irq_cause_w))                        // IRQ Cause
	map(0x478833, 0x478833).w(FUNC(metro_vdp3_state::irq_cause_w));                        // IRQ Acknowledge
	map(0x478836, 0x478837).nopw();                                        // ? watchdog ?
	map(0x478880, 0x478881).r(FUNC(metro_vdp3_state::gakusai_input_r));                           // Inputs
	...
	map(0x800001, 0x800001).w(FUNC(metro_vdp3_state::gakusai_oki_bank_hi_w));           // Samples Bank?
	map(0x900000, 0x900001).nopw();                                        // ? 4
	map(0xa00001, 0xa00001).w(FUNC(metro_vdp3_state::gakusai_oki_bank_lo_w));           // Samples Bank
	map(0xc00000, 0xc00003).w("ymsnd", FUNC(ym2413_device::write)).umask16(0x00ff);     //
...
void metro_vdp1_state::pangpoms_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400000, 0x47ffff).m(m_vdp, FUNC(imagetek_i4100_device::map));
	map(0x4788a3, 0x4788a3).rw(FUNC(metro_vdp1_state::irq_cause_r), FUNC(metro_vdp1_state::irq_cause_w));  // IRQ Cause / IRQ Acknowledge
	map(0x4788a4, 0x4788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0x4788a9, 0x4788a9).w(FUNC(metro_vdp1_state::soundlatch_w));                       // To Sound CPU
	map(0x800001, 0x800001).rw(FUNC(metro_vdp1_state::soundstatus_r), FUNC(metro_vdp1_state::soundstatus_w));  // From / To Sound CPU
	map(0x800002, 0x800003).nopr();
	map(0x800003, 0x800003).w(FUNC(metro_vdp1_state::coin_lockout_1word_w));    // Coin Lockout
...
void metro_vdp1_state::poitto_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400000, 0x40ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
	map(0x800000, 0x800001).portr("IN0");
	map(0x800001, 0x800001).w(FUNC(metro_vdp1_state::soundstatus_w));  // To Sound CPU
	map(0x800002, 0x800003).portr("IN1");                                //
	map(0x800004, 0x800005).portr("DSW0");                               //
	map(0x800006, 0x800007).portr("IN2");                                //
	map(0x800002, 0x800009).w(FUNC(metro_vdp1_state::coin_lockout_4words_w));              // Coin Lockout
	map(0xc00000, 0xc7ffff).m(m_vdp, FUNC(imagetek_i4100_device::map));
	map(0xc788a3, 0xc788a3).rw(FUNC(metro_vdp1_state::irq_cause_r), FUNC(metro_vdp1_state::irq_cause_w));  // IRQ Cause / IRQ Acknowledge
	map(0xc788a4, 0xc788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0xc788a9, 0xc788a9).w(FUNC(metro_vdp1_state::soundlatch_w));                       // To Sound CPU
}
...
void metro_vdp1_state::skyalert_map(address_map &map)
{
	map(0x000000, 0x03ffff).rom();                                             // ROM
	map(0x400001, 0x400001).rw(FUNC(metro_vdp1_state::soundstatus_r), FUNC(metro_vdp1_state::soundstatus_w));  // From / To Sound CPU
...
void metro_vdp2_state::pururun_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x400000, 0x400001).portr("IN0");
	map(0x400001, 0x400001).w(FUNC(metro_vdp2_state::soundstatus_w));  // To Sound CPU
	map(0x400002, 0x400003).portr("IN1");                                //
	map(0x400004, 0x400005).portr("DSW0");                               //
	map(0x400006, 0x400007).portr("IN2");                                //
	map(0x400002, 0x400009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
	map(0x800000, 0x80ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
	map(0xc00000, 0xc7ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0xc788a3, 0xc788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w));  // IRQ Cause / IRQ Acknowledge
	map(0xc788a4, 0xc788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0xc788a9, 0xc788a9).w(FUNC(metro_vdp2_state::soundlatch_w));                       // To Sound CPU
}
...
void metro_vdp2_state::toride2g_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x400000, 0x40ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
	map(0x800000, 0x800001).portr("IN0");  // Watchdog?
	map(0x800001, 0x800001).w(FUNC(metro_vdp2_state::soundstatus_w));  // To Sound CPU
	map(0x800002, 0x800003).portr("IN1");                                //
	map(0x800004, 0x800005).portr("DSW0");                               //
	map(0x800006, 0x800007).portr("IN2");                                //
	map(0x800002, 0x800009).w(FUNC(metro_vdp2_state::coin_lockout_4words_w));              // Coin Lockout
	map(0xc00000, 0xc7ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0xc788a3, 0xc788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	map(0xc788a4, 0xc788a5).writeonly().share("irq_enable");                // IRQ Enable
	map(0xc788a9, 0xc788a9).w(FUNC(metro_vdp2_state::soundlatch_w));                       // To Sound CPU
...
WRITE8_MEMBER(metro_base_state::blzntrnd_sh_bankswitch_w)
{
	m_audiobank->set_entry(data & 0x07);
}

void metro_vdp2_state::blzntrnd_sound_map(address_map &map)
{
...
void metro_vdp2_state::blzntrnd_sound_io_map(address_map &map)
{
	map.global_mask(0xff);
	map(0x00, 0x00).w(FUNC(metro_vdp2_state::blzntrnd_sh_bankswitch_w));
...
void metro_vdp2_state::blzntrnd_map(address_map &map)
{
	map(0x000000, 0x1fffff).rom();                                             // ROM
	map(0x200000, 0x27ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x2788a3, 0x2788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w));  // IRQ Cause / IRQ Acknowledge
	map(0x2788a4, 0x2788a5).writeonly().share("irq_enable");                // IRQ Enable

	map(0x400000, 0x43ffff).ram().w(FUNC(metro_vdp2_state::k053936_w)).share("k053936_ram");  // 053936
...
WRITE8_MEMBER(metro_base_state::mouja_sound_rombank_w)
{
	m_okibank->set_entry((data >> 3) & 0x07);
}

void metro_vdp3_state::mouja_map(address_map &map)
{
	map(0x000000, 0x07ffff).rom();                                             // ROM
	map(0x400000, 0x47ffff).m(m_vdp, FUNC(imagetek_i4300_device::v3_map));
	map(0x478810, 0x47881f).writeonly().share("irq_levels");                // IRQ Levels
	map(0x478820, 0x47882f).writeonly().share("irq_vectors");               // IRQ Vectors
	map(0x478830, 0x478831).writeonly().share("irq_enable");                // IRQ Enable
	map(0x478833, 0x478833).rw(FUNC(metro_vdp3_state::irq_cause_r), FUNC(metro_vdp3_state::irq_cause_w));  // IRQ Cause / IRQ Acknowledge
	map(0x478834, 0x478835).w(FUNC(metro_vdp3_state::mouja_irq_timer_ctrl_w));                   // IRQ set timer count
	...
	map(0x800001, 0x800001).w(FUNC(metro_vdp3_state::mouja_sound_rombank_w));
	map(0xc00000, 0xc00003).w("ymsnd", FUNC(ym2413_device::write)).umask16(0x00ff);
	map(0xd00000, 0xd00001).rw(m_oki, FUNC(okim6295_device::read), FUNC(okim6295_device::write));
	map(0xf00000, 0xf0ffff).ram().mirror(0x0f0000);                         // RAM (mirrored)
}

void metro_vdp3_state::mouja_okimap(address_map &map)
{
...
WRITE16_MEMBER(metro_base_state::puzzlet_irq_enable_w)
{
...
// H8/3007 CPU
void metro_vdp2_state::puzzlet_map(address_map &map)
{
	...
	// TODO: !!! i4300 !!!
	map(0x700000, 0x77ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x7788a3, 0x7788a3).w(FUNC(metro_vdp2_state::irq_cause_w));                            // IRQ Cause
	map(0x7788a4, 0x7788a5).w(FUNC(metro_vdp2_state::puzzlet_irq_enable_w)).share("irq_enable");  // IRQ Enable
	...
	map(0x7f88a2, 0x7f88a3).r(FUNC(metro_vdp2_state::irq_cause_r));                         // IRQ Cause
}


WRITE16_MEMBER(metro_base_state::puzzlet_portb_w)
{
//  popmessage("PORTB %02x", data);
}

void metro_vdp2_state::puzzlet_io_map(address_map &map)
{
	map(h8_device::PORT_7, h8_device::PORT_7).portr("IN2");
	map(h8_device::PORT_B, h8_device::PORT_B).portr("DSW0").w(FUNC(metro_vdp2_state::puzzlet_portb_w));
}
...
WRITE8_MEMBER(metro_base_state::vmetal_control_w)
{
...
WRITE8_MEMBER(metro_base_state::es8712_reset_w)
{
	m_essnd->reset();
}

WRITE_LINE_MEMBER(metro_base_state::vmetal_es8712_irq)
{
	if (m_essnd_gate)
		m_maincpu->set_input_line(3, state);
}

void metro_vdp2_state::vmetal_map(address_map &map)
{
	map(0x000000, 0x0fffff).rom();                                             // ROM
	map(0x100000, 0x17ffff).m(m_vdp, FUNC(imagetek_i4220_device::v2_map));
	map(0x1788a3, 0x1788a3).rw(FUNC(metro_vdp2_state::irq_cause_r), FUNC(metro_vdp2_state::irq_cause_w)); // IRQ Cause / IRQ Acknowledge
	...
	map(0x200001, 0x200001).w(FUNC(metro_vdp2_state::vmetal_control_w));
	...
	map(0x500000, 0x500000).w(FUNC(metro_vdp2_state::es8712_reset_w));
...
#define COINS_SOUND \
	...
	PORT_BIT(  0x0020, IP_ACTIVE_LOW,  IPT_START2   ) \
	PORT_BIT(  0x0040, IP_ACTIVE_HIGH, IPT_UNKNOWN  ) \
	PORT_BIT(  0x0080, IP_ACTIVE_HIGH, IPT_CUSTOM  ) PORT_CUSTOM_MEMBER(DEVICE_SELF, metro_base_state,custom_soundstatus_r, nullptr)   /* From Sound CPU */
...
void metro_base_state::machine_start()
{
...
void metro_base_state::base_config(machine_config &config)
{
...
void metro_vdp1_state::i4100_config(machine_config &config)
{
	base_config(config);

	I4100(config, m_vdp, 26.666_MHz_XTAL);
	m_vdp->blit_irq_cb().set(FUNC(metro_vdp1_state::vdp_blit_end_w));
}

void metro_vdp2_state::i4220_config(machine_config &config)
{
	base_config(config);

	I4220(config, m_vdp, 26.666_MHz_XTAL);
	m_vdp->blit_irq_cb().set(FUNC(metro_vdp2_state::vdp_blit_end_w));
}

void metro_vdp3_state::i4300_config(machine_config &config)
{
	base_config(config);

	I4300(config, m_vdp, 26.666_MHz_XTAL);
	m_vdp->blit_irq_cb().set(FUNC(metro_vdp3_state::vdp_blit_end_w));
}

// TODO: these comes from the CRTC inside the i4100
void metro_vdp1_state::i4100_config_360x224(machine_config &config)
{
	i4100_config(config);
...
void metro_vdp2_state::i4220_config_320x240(machine_config &config)
{
	i4220_config(config);
...
void metro_vdp2_state::i4220_config_304x224(machine_config &config)
{
	i4220_config(config);
...
void metro_vdp3_state::i4300_config_384x224(machine_config &config)
{
	i4300_config(config);
...
void metro_vdp3_state::i4300_config_320x240(machine_config &config)
{
	i4300_config(config);
...
void metro_vdp2_state::msgogo(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 16_MHz_XTAL);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::msgogo_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(60)); // ?

	/* video hardware */
	i4220_config(config);
	m_vdp->set_tmap_xoffsets(-2,-2,-2);

	m_screen->screen_vblank().set(FUNC(metro_vdp2_state::vblank_irq)); // timing is off, shaking sprites in intro

	/* sound hardware */
	SPEAKER(config, "mono").front_center();

	ymf278b_device &ymf(YMF278B(config, "ymf", 33.8688_MHz_XTAL));
	ymf.set_addrmap(0, &metro_vdp2_state::ymf278_map);
	ymf.irq_handler().set_inputline("maincpu", 2);
	ymf.add_route(ALL_OUTPUTS, "mono", 1.0);
}

void metro_vdp2_state::balcube(machine_config &config)
{
	msgogo(config);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::balcube_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?

	m_screen->screen_vblank().set(FUNC(metro_vdp2_state::vblank_irq));
}

void metro_vdp2_state::daitoa(machine_config &config)
{
	msgogo(config);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::daitoa_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?

	m_screen->screen_vblank().set(FUNC(metro_vdp2_state::vblank_irq));
}

void metro_vdp2_state::bangball(machine_config &config)
{
	msgogo(config);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::bangball_map);
	m_maincpu->set_periodic_int(device_interrupt_delegate(), attotime());
	TIMER(config, "scantimer").configure_scanline(FUNC(metro_vdp2_state::bangball_scanline), "screen", 0, 1);
...
void metro_vdp2_state::batlbubl(machine_config &config)
{
	msgogo(config);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::batlbubl_map);
	m_maincpu->set_periodic_int(device_interrupt_delegate(), attotime());
	TIMER(config, "scantimer").configure_scanline(FUNC(metro_vdp2_state::bangball_scanline), "screen", 0, 1);
...
void metro_base_state::metro_upd7810_sound(machine_config &config)
{
	upd7810_device &upd(UPD7810(config, m_audiocpu, 24_MHz_XTAL/2));
	upd.rxd_func().set(FUNC(metro_base_state::rxd_r));
	upd.set_addrmap(AS_PROGRAM, &metro_base_state::upd7810_map);
	upd.pa_in_cb().set(FUNC(metro_base_state::upd7810_porta_r));
	upd.pa_out_cb().set(FUNC(metro_base_state::upd7810_porta_w));
	upd.pb_out_cb().set(FUNC(metro_base_state::upd7810_portb_w));
	upd.pc_out_cb().set(FUNC(metro_base_state::upd7810_rombank_w<0x03>));
}

void metro_vdp2_state::daitorid_upd7810_sound(machine_config &config)
{
	upd7810_device &upd(UPD7810(config, m_audiocpu, 12_MHz_XTAL));
	upd.rxd_func().set(FUNC(metro_vdp2_state::rxd_r));
	upd.set_addrmap(AS_PROGRAM, &metro_vdp2_state::upd7810_map);
	upd.pa_in_cb().set(FUNC(metro_vdp2_state::upd7810_porta_r));
	upd.pa_out_cb().set(FUNC(metro_vdp2_state::upd7810_porta_w));
	upd.pb_out_cb().set(FUNC(metro_vdp2_state::daitorid_portb_w));
	upd.pc_out_cb().set(FUNC(metro_vdp2_state::upd7810_rombank_w<0x07>));
}

void metro_vdp2_state::daitorid(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 32_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::daitorid_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp2_state::puzzli(machine_config &config)
{
	daitorid(config);
...
void metro_vdp2_state::dharma(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::dharma_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp1_state::karatour(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp1_state::karatour_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp1_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
	...
	m_screen->screen_vblank().set(FUNC(metro_vdp1_state::karatour_vblank_irq));
...
void metro_vdp2_state::sankokushi(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::kokushi_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
	...
	m_screen->screen_vblank().set(FUNC(metro_vdp2_state::karatour_vblank_irq));
...
void metro_vdp1_state::lastfort(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp1_state::lastfort_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp1_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
	...
	m_screen->screen_vblank().set(FUNC(metro_vdp1_state::vblank_irq));
...
void metro_vdp1_state::lastforg(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp1_state::lastforg_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp1_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp3_state::dokyusei(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 16_MHz_XTAL);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp3_state::dokyusei_map);
	m_maincpu->set_addrmap(m68000_device::AS_CPU_SPACE, &metro_vdp3_state::cpu_space_map);

	/* video hardware */
	i4300_config(config);

	m_screen->screen_vblank().set(FUNC(metro_vdp3_state::vblank_irq));
...
void metro_vdp3_state::dokyusp(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 32_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp3_state::dokyusp_map);
	m_maincpu->set_addrmap(m68000_device::AS_CPU_SPACE, &metro_vdp3_state::cpu_space_map);
...
void metro_vdp3_state::gakusai(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 16000000); /* 26.6660MHz/2?, OSCs listed are 26.6660MHz & 3.579545MHz */
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp3_state::gakusai_map);
	m_maincpu->set_addrmap(m68000_device::AS_CPU_SPACE, &metro_vdp3_state::cpu_space_map);
...
void metro_vdp3_state::gakusai2(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 16000000); /* 26.6660MHz/2?, OSCs listed are 26.6660MHz & 3.579545MHz */
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp3_state::gakusai2_map);
	m_maincpu->set_addrmap(m68000_device::AS_CPU_SPACE, &metro_vdp3_state::cpu_space_map);
...
void metro_vdp1_state::pangpoms(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp1_state::pangpoms_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp1_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp1_state::poitto(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp1_state::poitto_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp1_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp2_state::pururun(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2); /* Not confirmed */
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::pururun_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp1_state::skyalert(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp1_state::skyalert_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp1_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp2_state::toride2g(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 24_MHz_XTAL/2);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::toride2g_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
...
void metro_vdp3_state::mouja(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 16_MHz_XTAL);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp3_state::mouja_map);
	m_maincpu->set_addrmap(m68000_device::AS_CPU_SPACE, &metro_vdp3_state::cpu_space_map);
	...
	m_oki->set_addrmap(0, &metro_vdp3_state::mouja_okimap);
	m_oki->add_route(ALL_OUTPUTS, "mono", 0.25);
...
void metro_vdp2_state::vmetal(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 16_MHz_XTAL);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::vmetal_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?
	...
	m_vdp->set_tmap_xoffsets(-16,-16,-16);
	m_vdp->set_tmap_yoffsets(-16,-16,-16);
	...
	m_essnd->reset_handler().set(FUNC(metro_vdp2_state::vmetal_es8712_irq));
...
void metro_vdp2_state::blzntrnd(machine_config &config)
{
	/* basic machine hardware */
	M68000(config, m_maincpu, 16_MHz_XTAL);
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::blzntrnd_map);
	m_maincpu->set_periodic_int(FUNC(metro_vdp2_state::periodic_interrupt), attotime::from_hz(8*60)); // ?

	Z80(config, m_audiocpu, 16_MHz_XTAL/2);
	m_audiocpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::blzntrnd_sound_map);
	m_audiocpu->set_addrmap(AS_IO, &metro_vdp2_state::blzntrnd_sound_io_map);
	...
	m_screen->set_size(320, 240);
	m_screen->set_visarea(0, 304-1, 0, 224-1);
	m_screen->set_screen_update(FUNC(metro_vdp2_state::screen_update_psac_vdp2_mix));
	m_screen->screen_vblank().set(FUNC(metro_vdp2_state::karatour_vblank_irq));

	MCFG_VIDEO_START_OVERRIDE(metro_vdp2_state,blzntrnd)

	GFXDECODE(config, m_gfxdecode, "vdp:palette", gfx_blzntrnd);
...
void metro_vdp2_state::gstrik2(machine_config &config)
{
	blzntrnd(config);
	m_gfxdecode->set_info(gfx_gstrik2);

	MCFG_VIDEO_START_OVERRIDE(metro_vdp2_state,gstrik2)
...
void metro_vdp2_state::puzzlet(machine_config &config)
{
	/* basic machine hardware */
	H83007(config, m_maincpu, 20_MHz_XTAL); // H8/3007 - Hitachi HD6413007F20 CPU. Clock 20MHz
	m_maincpu->set_addrmap(AS_PROGRAM, &metro_vdp2_state::puzzlet_map);
	m_maincpu->set_addrmap(AS_IO, &metro_vdp2_state::puzzlet_io_map);
	...
	m_screen->screen_vblank().set(FUNC(metro_vdp2_state::puzzlet_vblank_irq));
...
ROM_START( batlbubl )
	...
	ROM_REGION( 0x800000, "vdp", 0 )   /* Gfx + Data (Addressable by CPU & Blitter) */
...
ROM_START( blzntrnd )
	...
	ROM_REGION( 0x1800000, "vdp", 0 )  /* Gfx + Data (Addressable by CPU & Blitter) */
...
ROM_START( gstrik2 )
	...
	ROM_REGION( 0x1000000, "vdp", 0 )  /* Gfx + Data (Addressable by CPU & Blitter) */
...
ROM_START( dharmak )
	...
	ROM_REGION( 0x200000, "vdp", 0 )   /* Gfx + Data (Addressable by CPU & Blitter) */ /* note, these are bitswapped, see init */
...
ROM_START( gakusai )
	...
	ROM_REGION( 0x2000000, "vdp", 0 )  /* Gfx + Data (Addressable by CPU & Blitter) */
...
void metro_base_state::metro_common(  )
{
...
void metro_base_state::init_metro()
{
	metro_common();
...
void metro_base_state::init_karatour()
{
...
void metro_base_state::init_balcube()
{
	uint8_t *ROM       = memregion("vdp")->base();
	const unsigned len = memregion("vdp")->bytes();
...
void metro_base_state::init_dharmak()
{
	uint8_t *src = memregion("vdp")->base();
...
void metro_base_state::init_blzntrnd()
{
	metro_common();
	m_irq_line = 1;
...
void metro_base_state::init_vmetal()
{
	metro_common();
	m_irq_line = 1;
	m_essnd_gate = false;
...
void metro_base_state::init_mouja()
{
	metro_common();
...
void metro_base_state::init_gakusai()
{
	metro_common();
	m_irq_line = -1;
	m_vblank_bit = 1;
...
void metro_base_state::init_puzzlet()
{
	metro_common();
	m_irq_line = 0;
	m_vblank_bit = 1;
...
void metro_base_state::init_lastfortg()
{
	init_metro();
	m_karatour_irq_timer = timer_alloc(TIMER_KARATOUR_IRQ);
}
...
// VG420 / VG460
GAME( 1992, karatour,  0,        karatour,  karatour,   metro_vdp1_state, init_karatour, ROT0,   "Mitchell",                                        "The Karate Tournament", MACHINE_SUPPORTS_SAVE )
GAME( 1992, karatourj, karatour, karatour,  karatour,   metro_vdp1_state, init_karatour, ROT0,   "Mitchell",                                        "Chatan Yarakuu Shanku - The Karate Tournament (Japan)", MACHINE_SUPPORTS_SAVE )
GAME( 1992, pangpoms,  0,        pangpoms,  pangpoms,   metro_vdp1_state, init_metro,    ROT0,   "Metro",                                           "Pang Pom's", MACHINE_SUPPORTS_SAVE )
GAME( 1992, pangpomsm, pangpoms, pangpoms,  pangpoms,   metro_vdp1_state, init_metro,    ROT0,   "Metro (Mitchell license)",                        "Pang Pom's (Mitchell)", MACHINE_SUPPORTS_SAVE )
GAME( 1992, pangpomsn, pangpoms, pangpoms,  pangpoms,   metro_vdp1_state, init_metro,    ROT0,   "Nova",                                            "Pang Pom's (Nova)", MACHINE_SUPPORTS_SAVE )
GAME( 1992, skyalert,  0,        skyalert,  skyalert,   metro_vdp1_state, init_metro,    ROT270, "Metro",                                           "Sky Alert", MACHINE_SUPPORTS_SAVE )
GAME( 1993, ladykill,  0,        karatour,  ladykill,   metro_vdp1_state, init_karatour, ROT90,  "Yanyaka (Mitchell license)",                      "Lady Killer", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1993, moegonta,  ladykill, karatour,  moegonta,   metro_vdp1_state, init_karatour, ROT90,  "Yanyaka",                                         "Moeyo Gonta!! (Japan)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1994, lastfort,  0,        lastfort,  lastfort,   metro_vdp1_state, init_metro,    ROT0,   "Metro",                                           "Last Fortress - Toride (Japan, VG420 PCB)", MACHINE_SUPPORTS_SAVE ) // VG420 PCB
GAME( 1994, lastforte, lastfort, lastfort,  lastfero,   metro_vdp1_state, init_metro,    ROT0,   "Metro",                                           "Last Fortress - Toride (China, Rev C)", MACHINE_SUPPORTS_SAVE )
GAME( 1994, lastfortea,lastfort, lastfort,  lastfero,   metro_vdp1_state, init_metro,    ROT0,   "Metro",                                           "Last Fortress - Toride (China, Rev A)", MACHINE_SUPPORTS_SAVE )
GAME( 1994, lastfortk, lastfort, lastfort,  lastfero,   metro_vdp1_state, init_metro,    ROT0,   "Metro",                                           "Last Fortress - Toride (Korea)", MACHINE_SUPPORTS_SAVE )
GAME( 1994, lastfortj, lastfort, lastforg,  ladykill,   metro_vdp1_state, init_lastfortg,ROT0,   "Metro",                                           "Last Fortress - Toride (Japan, VG460 PCB)", MACHINE_SUPPORTS_SAVE ) // VG460-(A) PCB
GAME( 1994, lastfortg, lastfort, lastforg,  ladykill,   metro_vdp1_state, init_lastfortg,ROT0,   "Metro",                                           "Last Fortress - Toride (Germany)", MACHINE_SUPPORTS_SAVE ) // VG460-(A) PCB

// MTR5260 / MTR527
GAME( 1993, poitto,    0,        poitto,    poitto,     metro_vdp1_state, init_metro,    ROT0,   "Metro / Able Corp.",                              "Poitto!", MACHINE_SUPPORTS_SAVE )
GAME( 1994, dharma,    0,        dharma,    dharma,     metro_vdp2_state, init_dharmak,  ROT0,   "Metro",                                           "Dharma Doujou", MACHINE_SUPPORTS_SAVE )
GAME( 1994, dharmaj,   dharma,   dharma,    dharma,     metro_vdp2_state, init_metro,    ROT0,   "Metro",                                           "Dharma Doujou (Japan)", MACHINE_SUPPORTS_SAVE )
GAME( 1994, dharmak,   dharma,   dharma,    dharma,     metro_vdp2_state, init_dharmak,  ROT0,   "Metro",                                           "Dharma Doujou (Korea)", MACHINE_SUPPORTS_SAVE )
GAME( 1994, toride2g,  0,        toride2g,  toride2g,   metro_vdp2_state, init_metro,    ROT0,   "Metro",                                           "Toride II Adauchi Gaiden", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1994, toride2gg, toride2g, toride2g,  toride2g,   metro_vdp2_state, init_metro,    ROT0,   "Metro",                                           "Toride II Adauchi Gaiden (German)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1994, toride2gk, toride2g, toride2g,  toride2g,   metro_vdp2_state, init_metro,    ROT0,   "Metro",                                           "Toride II Bok Su Oi Jeon Adauchi Gaiden (Korea)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1994, toride2j,  toride2g, toride2g,  toride2g,   metro_vdp2_state, init_metro,    ROT0,   "Metro",                                           "Toride II (Japan)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1994, gunmast,   0,        pururun,   gunmast,    metro_vdp2_state, init_metro,    ROT0,   "Metro",                                           "Gun Master", MACHINE_SUPPORTS_SAVE )
GAME( 1995, daitorid,  0,        daitorid,  daitorid,   metro_vdp2_state, init_metro,    ROT0,   "Metro",                                           "Daitoride", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1995, pururun,   0,        pururun,   pururun,    metro_vdp2_state, init_metro,    ROT0,   "Metro / Banpresto",                               "Pururun", MACHINE_SUPPORTS_SAVE )
GAME( 1995, puzzli,    0,        puzzli,    puzzli,     metro_vdp2_state, init_metro,    ROT0,   "Metro / Banpresto",                               "Puzzli",      MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1996, 3kokushi,  0,        sankokushi,sankokushi, metro_vdp2_state, init_karatour, ROT0,   "Mitchell",                                        "Sankokushi (Japan)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )

// ? with additional gfx data scramble (probably MTR5260 based)
GAME( 1995, msgogo,    0,        msgogo,    msgogo,     metro_vdp2_state, init_balcube,  ROT0,   "Metro",                                           "Mouse Shooter GoGo", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1996, daitorida, daitorid, daitoa,    daitorid,   metro_vdp2_state, init_balcube,  ROT0,   "Metro",                                           "Daitoride (YMF278B version)", MACHINE_SUPPORTS_SAVE )
GAME( 1996, balcube,   0,        balcube,   balcube,    metro_vdp2_state, init_balcube,  ROT0,   "Metro",                                           "Bal Cube", MACHINE_SUPPORTS_SAVE )
GAME( 1996, bangball,  0,        bangball,  bangball,   metro_vdp2_state, init_balcube,  ROT0,   "Banpresto / Kunihiko Tashiro+Goodhouse",          "Bang Bang Ball (v1.05)", MACHINE_SUPPORTS_SAVE )
GAME( 1999, batlbubl,  bangball, batlbubl,  batlbubl,   metro_vdp2_state, init_balcube,  ROT0,   "Banpresto (Limenko license?)",                    "Battle Bubble (v2.00)", MACHINE_SUPPORTS_SAVE ) // or bootleg?

// VG330 / VG340 / VG410
GAME( 1995, dokyusei,  0,        dokyusei,  dokyusei,   metro_vdp3_state, init_gakusai,  ROT0,   "Make Software / Elf / Media Trading",             "Mahjong Doukyuusei", MACHINE_SUPPORTS_SAVE )
GAME( 1995, dokyusp,   0,        dokyusp,   gakusai,    metro_vdp3_state, init_gakusai,  ROT0,   "Make Software / Elf / Media Trading",             "Mahjong Doukyuusei Special", MACHINE_SUPPORTS_SAVE )
GAME( 1996, mouja,     0,        mouja,     mouja,      metro_vdp3_state, init_mouja,    ROT0,   "Etona",                                           "Mouja (Japan)", MACHINE_SUPPORTS_SAVE )
GAME( 1997, gakusai,   0,        gakusai,   gakusai,    metro_vdp3_state, init_gakusai,  ROT0,   "MakeSoft",                                        "Mahjong Gakuensai (Japan)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE )
GAME( 1998, gakusai2,  0,        gakusai2,  gakusai,    metro_vdp3_state, init_gakusai,  ROT0,   "MakeSoft",                                        "Mahjong Gakuensai 2 (Japan)", MACHINE_SUPPORTS_SAVE )

// HUM-002 / HUM-003
GAME( 1994, blzntrnd,  0,        blzntrnd,  blzntrnd,   metro_vdp2_state, init_blzntrnd, ROT0,   "Human Amusement",                                 "Blazing Tornado", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE | MACHINE_NO_COCKTAIL )
GAME( 1996, gstrik2,   0,        gstrik2,   gstrik2,    metro_vdp2_state, init_blzntrnd, ROT0,   "Human Amusement",                                 "Grand Striker 2 (Europe and Oceania)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE | MACHINE_NO_COCKTAIL )
GAME( 1996, gstrik2j,  gstrik2,  gstrik2,   gstrik2,    metro_vdp2_state, init_blzntrnd, ROT0,   "Human Amusement",                                 "Grand Striker 2 (Japan)", MACHINE_IMPERFECT_GRAPHICS | MACHINE_SUPPORTS_SAVE | MACHINE_NO_COCKTAIL ) // priority between rounds

// ES-9309B-B
GAME( 1995, vmetal,    0,        vmetal,    vmetal,     metro_vdp2_state, init_vmetal,   ROT90,  "Excellent System",                                "Varia Metal", MACHINE_SUPPORTS_SAVE )
GAME( 1995, vmetaln,   vmetal,   vmetal,    vmetal,     metro_vdp2_state, init_vmetal,   ROT90,  "Excellent System (New Ways Trading Co. license)", "Varia Metal (New Ways Trading Co.)", MACHINE_SUPPORTS_SAVE )

// VG2200
GAME( 2000, puzzlet,   0,        puzzlet,   puzzlet,    metro_vdp2_state, init_puzzlet,  ROT0,   "Unies Corporation",                               "Puzzlet (Japan)", MACHINE_NOT_WORKING | MACHINE_NO_SOUND | MACHINE_SUPPORTS_SAVE )
