...
public:
	...
	const pen_t *pens() { return m_palette->pens(); }
...
protected:
	...
	void draw_layers( screen_device &screen, bitmap_argb32 &bitmap, const rectangle &cliprect, int pri );
	inline uint8_t get_tile_pix( uint16_t code, uint8_t x, uint8_t y, bool const big, uint16_t *pix );
	void draw_tilemap( screen_device &screen, bitmap_argb32 &bitmap, const rectangle &cliprect, uint32_t flags, uint32_t const pcode,
					int sx, int sy, int wx, int wy, bool const big, uint16_t const *tilemapram, int const layer );
	void draw_spritegfx(screen_device &screen, bitmap_argb32 &bitmap, const rectangle &clip,
